...
void mine_proc(HeadTable &ht){
    int j = 0;
    for(int itmp=0;itmp<ht.size;itmp++){
...
void preprocess(HeadTable &ht,IHUP &ihup,char* transaction_file_name,int offset,int offset2){
//第三遍数据库的扫描,插入rescan_nodes.
    //根据ht建立好新的索引
    int i, j, k, *buf, numitem, tid;
    int blk = offset;
   int lb = 0;
   ...
   for (i = lb; i < ub;i++){
	   ...
	   //插入操作;
           tid = tid-1;
           ihup.reinsert(ihuproot, tid, item_rank_index, ht,offset2);
           //cout<<"trans "<<tid<<" complete"<<endl;
           init_rank();
   }
...
max_trans_sz = Database_readfrom(transaction_file_name2,offset);  //更新了transaction_utility_array,item_t_utility
if (max_trans_sz < tmp_max_trans_sz){
    max_trans_sz = tmp_max_trans_sz;
}

vector <int> delete_nodes;
vector <int> rescan_nodes;
vector< int >::iterator rescanIterator ;
    
    for(int i = 0; i<maxitem ;++i){
        if(item_t_utility[i] >= MIN_UTILITY){
        rescan_nodes.push_back(i);
        }
    }


    
    //比较newht和oldht的不同，将newht指向oldht的指针
    for(int i = ht.size -1 ; i > -1; --i){
        ...
        while( rescanIterator != rescan_nodes.end() ){
            if((*rescanIterator) == ht.HeadNode[i].item2){
                rescanIterator = rescan_nodes.erase(rescanIterator);
            }else{
                ++rescanIterator;
            }
        }
...
int newhtsize = rescan_nodes.size();
//初始化新的headtable
if(rescan_nodes.size() > 0){
    int size_offset = ht.size;
    ht.resize(rescan_nodes.size());
    for(int ii =0;ii<rescan_nodes.size();++ii){
        int i = ii+size_offset;
        ht.HeadNode[i].item2 = rescan_nodes[ii];
        ht.HeadNode[i].t_utility = item_t_utility[rescan_nodes[ii]];
        ht.HeadNode[i].tf = 0;
        ht.HeadNode[i].linknode = NULL;
    }
